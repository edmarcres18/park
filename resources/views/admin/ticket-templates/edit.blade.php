...
@section('subtitle', 'Update ticket template')
...
<div class="space-y-6">
        ...
        <div class="bg-gradient-to-r from-blue-600 to-purple-600 px-6 py-4">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-3">
                    ...
                    <div>
                        <h2 class="text-xl font-bold text-white">Edit Template</h2>
                        <p class="text-blue-100 text-sm">Update ticket printing template</p>
                    </div>
                </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div class="px-6 py-8 sm:px-10">
            <form action="{{ route('admin.ticket-templates.update', $template) }}" method="POST" class="space-y-8">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <textarea name="html_template" id="html_template" rows="15"
                              ...
                              placeholder="Enter HTML template with variables like {{ticket_number}}, {{plate_number}}, etc." required>{{ old('html_template', $template->html_template) }}</textarea>
                    @error('html_template')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <textarea name="css_styles" id="css_styles" rows="8"
                              class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 font-mono text-sm"
                              placeholder="Enter custom CSS styles for the template">{{ old('css_styles', $template->css_styles) }}</textarea>
                    @error('css_styles')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="flex items-center">
                            <input type="checkbox" name="template_variables[]" id="var_{{ $key }}" value="{{ $key }}"
                                   ...
                                   {{ in_array($key, old('template_variables', $template->template_variables ?? [])) ? 'checked' : '' }}>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $template->sort_order) }}"
                               class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    </div>
                </div>
