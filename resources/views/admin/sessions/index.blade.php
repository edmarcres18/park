...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        <div class="flex items-center space-x-4">
            ...
            <div class="bg-white/80 backdrop-blur-sm rounded-xl px-4 py-2 shadow-lg">
                <span class="text-sm text-slate-600">Active: <span class="font-semibold text-green-600">{{ $sessions->filter(fn($s) => $s->isActive())->count() }}</span></span>
            </div>
        </div>
        <a href="{{ route('admin.sessions.create') }}" class="bg-gradient-to-r from-blue-600 to-purple-600 text-white px-6 py-3 rounded-xl hover:from-blue-700 hover:to-purple-700 transition-all duration-200 font-medium shadow-lg flex items-center space-x-2">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div class="bg-white/90 backdrop-blur-sm rounded-2xl shadow-xl p-6 border border-blue-200">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-blue-900">{{ $sessions->where('end_time', null)->count() }}</p>
                </div>
        ...
        <div class="bg-white/90 backdrop-blur-sm rounded-2xl shadow-xl p-6 border border-purple-200">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-purple-600 text-sm font-medium">Today's Revenue</p>
                    <p class="text-3xl font-bold text-purple-900">₱{{ number_format($sessions->where('created_at', '>=', today())->sum('amount_paid'), 2) }}</p>
                </div>
    ...
    <div class="bg-white/90 backdrop-blur-sm rounded-2xl shadow-xl overflow-hidden">
        ...
        <div class="border-b border-slate-200">
            <nav class="flex space-x-8 px-6" aria-label="Tabs">
                <button class="tab-button active border-b-2 border-blue-500 py-4 px-1 text-sm font-medium text-blue-600" data-tab="all">
        ...
        <div class="tab-content active" id="all-content">
            @include('admin.sessions.partials.sessions-table', ['sessions' => $sessions])
        </div>
        
        <div class="tab-content hidden" id="active-content">
            @include('admin.sessions.partials.sessions-table', ['sessions' => $sessions->whereNull('end_time')])
        </div>
        
        <div class="tab-content hidden" id="completed-content">
            @include('admin.sessions.partials.sessions-table', ['sessions' => $sessions->whereNotNull('end_time')])
        </div>
    </div>
</div>
