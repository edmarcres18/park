...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="bg-white/80 backdrop-blur-xl border border-white/60 rounded-2xl shadow-xl p-6">
        <div class="flex items-center gap-4 mb-4">
            <img src="https://ui-avatars.com/api/?name={{ urlencode($user->name) }}&background=3b82f6&color=fff&rounded=true" alt="Avatar" class="w-12 h-12 rounded-full" />
            <div>
        ...
        <form method="POST" action="{{ route('profile.update') }}" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input id="name" name="name" type="text" value="{{ old('name', $user->name) }}" required
                           class="mt-1 w-full px-3 py-2 rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white/70" />
                    @error('name')
            ...
            <div class="flex items-center justify-end gap-3">
                ...
                <button type="submit" class="px-5 py-2 rounded-lg text-white bg-blue-600 hover:bg-blue-700 transition">Save Changes</button>
            </div>
        </form>
    </div>
</div>
